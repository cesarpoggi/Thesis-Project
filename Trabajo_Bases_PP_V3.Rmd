---
title: "Tesis - Data Preliminar (Con 2023)"
author: "Cesar Poggi"
date: "2023-10-01"
...
PP2023_pag_html <- "https://www.dropbox.com/scl/fi/x2yucic9b2m9q9idne21t/agentes_participantes_2023_pagina.csv?rlkey=dcmtd0unx987lcaw5zov4a8js&dl=1"
```
...
PP2023_pag=import(PP2023_pag_html, encoding = 'UTF-8')
```
...
PP2023_pag <- select(PP2023_pag, c("orgn"))
```
...
PP2020_pag$AÑO2023 <- c("0")

PP2023_pag$AÑO2015 <- c("0") 
PP2023_pag$AÑO2016 <- c("0") 
PP2023_pag$AÑO2017 <- c("0") 
PP2023_pag$AÑO2018 <- c("0") 
PP2023_pag$AÑO2020 <- c("0")
PP2023_pag$AÑO2023 <- c("1")
```
...
PP2023_pag$orgn <- iconv(PP2023_pag$orgn, to='ASCII//TRANSLIT')
```

Merge de bases
```{r}
df_lista <- list(PP2015_pag, PP2016_pag, PP2017_pag, PP2018_pag, PP2020_pag, PP2023_pag)
...
PP_FULL$index_OG_col <- as.numeric(PP_FULL$index_OG_col)
...
PP_FULL1 <- PP_FULL %>%
    separate_rows(orgn, sep = " ")%>%
    group_by(index_OG_col) %>%
    slice_min(match(orgn, reference_importance)) %>%
  rename(tipo_orgn_nom = orgn) %>%
  select(tipo_orgn_nom,index_OG_col)


n_occur_FULL1 <- data.frame(table(PP_FULL1$index_OG_col)) #casos con doble repetición son casos que tienen dos palabras clave de la misma jerarquía (o que son iguales?), eliminarlos manualmente antes del merge -> Identificarlos como Var1 de n_occur_FULL1 refiere al rowid de PP_FULL1 y con el index_OG_col identificamos el caso que genera dos filas diferentes por repetición de palabra en la misma jerarquía (la misma palabra)

PP_FULL1 <- PP_FULL1 %>%
  filter(!duplicated(index_OG_col)) #Diferencia de 2 filas con la data original, significa que hay una duplicacion en la data original (con esto eliminamos el problema anterior, ya no lo tenemos por qué hacer manualmente) Estamos seleccionando los NO duplicados
```

Sacamos otra vez el rowid de PP_FULL para poder mergearlo con PP_FULL1 y tener las columnas de tipo_org_nom
```{r}
PP_FULL2 <- PP_FULL %>% 
  #rowid_to_column() %>% 
  left_join(PP_FULL1,by ="index_OG_col")

n_occur_FULL2 <- data.frame(table(PP_FULL2$index_OG_col))
```
         ...
         ifelse(PP_FULL2$tipo_orgn_nom %in% c("CULTURAL","CULTURA"), "CT",
                ifelse(PP_FULL2$tipo_orgn_nom %in% c("ADULTO","JUBILADOS","EDAD","TERCERA"), "SR",
                       ifelse(PP_FULL2$tipo_orgn_nom == "MUNICIPALIDAD", "MN",
                              ifelse(PP_FULL2$tipo_orgn_nom %in% c("COMEDORES","COMEDOR", "COMEDORSARITA","DICECOA","OSBCOMEDOR","4868"),"CP",
                                     ifelse(PP_FULL2$tipo_orgn_nom %in% c("VASO","VDL","PVL"),"VL",
                                            ifelse(PP_FULL2$tipo_orgn_nom %in% c("MADRES","MADRE","MADRESNUESTRO","BENDICION"),"CM",
                                                   ifelse(PP_FULL2$tipo_orgn_nom %in% c("COORDINADORA","COORDINAROA","CORDINADORA","COORDIANDORA"), "CD",
                                                          ifelse(PP_FULL2$tipo_orgn_nom %in% c("ASOCDE","ASOCTRABJ","TRABAJADORES", "ASOCTRABAJADORES", "RECICLADORES","BOMBEROS","TRANSPORTISTAS","TRANSPORTE","MERCADO","COMERCANTES","MCDO","COMERCIANTES","SUTRAMUVES","MOTOTAXISTAS","AUTOMOTRICES","CONSTRUCCIVIL","CONAFOVICER"), "TB",
                                                                 ifelse(PP_FULL2$tipo_orgn_nom %in% c("EMPRESARIOS","MICROEMPRESARIOS","SAC","EIRL","ESTACIONAMIENTO","APEMIC","APEMIVES","APEMINAVES","APIMEAVES", "INDUSTRIALES","ALIMENTARIA"),"EM",
                                                                        ifelse(PP_FULL2$tipo_orgn_nom %in% c("EDUCAION","EDUCACION","EDUCATIVO","IE","IEN","IESTP","CETPRO","APEMICOVES","ESCOLARIZADO","GOTITAS"), "ED",
                                                                               ifelse(PP_FULL2$tipo_orgn_nom %in% c("AMBIENTE","AMBIENTAL","VERDES","VERDE","NATURALES","SOSTENIBLE","ECOLOGIA"),"MA",
                                                                                      ifelse(PP_FULL2$tipo_orgn_nom %in% c("APROCUAVES","APROSCUAVES","SALUD","APROSCUAVESASOCIACION"),"SA",
                                                                                             ifelse(PP_FULL2$tipo_orgn_nom %in%
                                                                                                    ...
                                                                                                    ifelse(PP_FULL2$tipo_orgn_nom %in% c("VECINOS","VECINAL","Vecinal","JJ", "POBLADORES","VECINA","VECINALES","COMUNAL","UPIS"), "VC",
                                                                                                           ifelse(PP_FULL2$tipo_orgn_nom %in%  c("PROPIETARIOS","ADJUDICATARIOS","ADJUDICARIOS","ASOCADJCONJHABPACHACAMAC","ADJ","POSECIONARIOS","POSESIONARIOS","ASPROREP"), "PR",
                                                                                                                  ifelse(PP_FULL2$tipo_orgn_nom %in% c("POSESIONARIOS","ASOC.VIV","ASOCVIVIENDA","VIVIENDA","ASOCVIV","ASOCVIVISTA","ASOCVIVSTA","HABITACIONAL","VIVI","VIV"), "VV",
                                                                                                                  ifelse(PP_FULL2$tipo_orgn_nom == "SEGURIDAD", "SG", 
                                                                                                                         ifelse(PP_FULL2$tipo_orgn_nom %in% c("OBRAS","PISTAS","RIESGOS","RIESGO","PROSERVICIOS","ASFALTADO","AGUA"), "OB",
                                                                                                                                ifelse(PP_FULL2$tipo_orgn_nom %in% c("AGROPECUARIO","AGROPECUARIA","AGROINDUSTRIAL","AGROP","AGROP18","AGROSILVES","AGR","AGROPAGROSILVES","MENSAJERA","AGRPECUARIOS","AGROPAGROWUSSINES","VELASCO"),'AG',
                                                                                                                                       ifelse(PP_FULL2$tipo_orgn_nom %in% c("HERMANDAD","REFORMADOR","3ACOMUNIDAD"), "RE",
                                                                                                                                              ifelse(PP_FULL2$tipo_orgn_nom %in% c("MUJERES","FEPOMUVES","CORDIMUVES","DIRIGENTAS","CLOTILDES","030MUJERESCOL"), "GE",
                                                                                                                                                     ifelse(PP_FULL2$tipo_orgn_nom %in% c("AAHH","ASENTAMIENTO","AH","AMPAAHH","PPJJ","AA","HH","PUEBLO"), "AH",
                                                                                                                                                            ifelse(PP_FULL2$tipo_orgn_nom %in% c("AMP","AP","AH","AMPLIACION","URBANIZACION","URB","ST","SECTOR","GR","GRUPO","Grupo","CALIFORNIA","DIRECTIVA"), "UR", "OO"))))))))))))))))))))))))


PP_FULL2$tipo_orgn_mod[748]<-"CM"
PP_FULL2$tipo_orgn_mod[1534]<-"CM"
PP_FULL2$tipo_orgn_mod[1613]<-"CM"
PP_FULL2$tipo_orgn_mod[153]<-"CM"
PP_FULL2$tipo_orgn_mod[156]<-"CM"
PP_FULL2$tipo_orgn_mod[157]<-"CM"
PP_FULL2$tipo_orgn_mod[1214]<-"CM"
PP_FULL2$tipo_orgn_mod[1216]<-"CM"
PP_FULL2$tipo_orgn_mod[1458]<-"CM"
PP_FULL2$tipo_orgn_mod[1682]<-"CM"
PP_FULL2$tipo_orgn_mod[2538]<-"CM"
PP_FULL2$tipo_orgn_mod[2541]<-"CM"
PP_FULL2$tipo_orgn_mod[1564]<-"CM"
PP_FULL2$tipo_orgn_mod[1617]<-"CM"
PP_FULL2$tipo_orgn_mod[1141]<-"CM"
PP_FULL2$tipo_orgn_mod[1142]<-"CM"

PP_FULL2$tipo_orgn_mod[2250]<-"OB"
PP_FULL2$tipo_orgn_mod[2837]<-"OB"
PP_FULL2$tipo_orgn_mod[2846]<-"OB"

PP_FULL2$tipo_orgn_mod[326:327]<-"VL"
PP_FULL2$tipo_orgn_mod[1350:1351]<-"VL"

PP_FULL2$tipo_orgn_mod[2302:2303]<-"VL"

PP_FULL2$tipo_orgn_mod[1821:1822]<-"SG"
PP_FULL2$tipo_orgn_mod[1348:1349]<-"SG"

PP_FULL2$tipo_orgn_mod[405:406]<-"SG"
PP_FULL2$tipo_orgn_mod[1355:1356]<-"SG"


PP_FULL2$tipo_orgn_mod[2304:2305]<-"SG"

PP_FULL2$tipo_orgn_mod[86:87]<-"OO"

PP_FULL2$tipo_orgn_mod[993:994]<-"OO"

PP_FULL2$tipo_orgn_mod[2893]<-"PR"
PP_FULL2$tipo_orgn_mod[2918]<-"PR"

PP_FULL2$tipo_orgn_mod[3120]<-"PR"

PP_FULL2$tipo_orgn_mod[3234:3235]<-"MA"

PP_FULL2$tipo_orgn_mod[3306:3307]<-"UR"

PP_FULL2$tipo_orgn_mod[3026:3027]<-"EM"

PP_FULL2$tipo_orgn_mod[3159:3160]<-"EM"
```
...
```{r}
PP_SUBSET[,c(2:7)] <- sapply(PP_SUBSET[c(2:7)],as.numeric)
...
Resumen_Tipo_Org <- Resumen_Tipo_Org[order(Resumen_Tipo_Org$Total, decreasing = TRUE),]
```

Exportar Tablas General
```{r}
write.table(Resumen_Tipo_Org, file = "Tabla_General.txt", sep = ",", quote = FALSE, row.names = FALSE)
```

Gráficos
```{r}
TablaGraf1 <- as.data.frame(t(Resumen_Tipo_Org))
colnames(TablaGraf1) <- TablaGraf1[c(1),]
TablaGraf1 <- TablaGraf1[-c(1),]
TablaGraf1$año <- row.names(TablaGraf1)
TablaGraf1 <- TablaGraf1[c(27,1:26)]
TablaGraf1 <- TablaGraf1[-c(7),]
TablaGraf1[,c(2:27)] <- sapply(TablaGraf1[c(2:27)],as.numeric)

TablaGraf1$media = TablaGraf1$Total/25
TablaGraf1$mediana <- apply(TablaGraf1[,2:26], 1, median, na.rm=T)
```


```{r}
#1. Grafica de agentes participantes a través de los años

library(plotly)
fig1 <- plot_ly(TablaGraf1, type = 'scatter', mode = 'lines+markers')%>%
  add_trace(x = ~año, y = ~Total, name = 'Variación', width = 4)
fig1 <- fig1 %>% layout(title = 'Variación de Agentes Participantes', plot_bgcolor = "#e5ecf6")
fig1 <- fig1 %>% layout(xaxis = list(title = 'Proceso de Presupuesto Participativo'))
fig1 <- fig1 %>% layout(yaxis = list(title = 'Total de Agentes Participantes'))
fig1
```


```{r}
#2. Media y Mediana anual de la cantidad de agentes participantes por organización

library(plotly)
fig1.1 <- plot_ly(TablaGraf1, type = 'scatter', mode = 'lines+markers')%>%
  add_trace(x = ~año, y = ~media, name = 'Media', width = 4)%>%
  add_trace(x = ~año, y = ~mediana, name = 'Mediana', width = 4)
fig1.1 <- fig1.1 %>% layout(title = 'Media y Mediana anual de Participación en el PP', plot_bgcolor = "#e5ecf6")
fig1.1 <- fig1.1 %>% layout(xaxis = list(title = 'Proceso de Presupuesto Participativo'))
fig1.1 <- fig1.1 %>% layout(yaxis = list(title = 'Agentes Participantes'))
fig1.1
```

```{r}
#3. Media y Mediana anual de la cantidad de agentes participantes por tipo organización + Participación por tipo de organización 

library(plotly)
fig2 <- plot_ly(TablaGraf1, type = 'scatter', mode = 'lines+markers')%>%
  add_trace(x = ~año, y = ~UR, name = 'Urbano', width = 4)%>%
  add_trace(x = ~año, y = ~CP, name = 'Comedor', width = 4)%>%
  add_trace(x = ~año, y = ~CM, name = 'Club de Madres', width = 4)%>%
  add_trace(x = ~año, y = ~VC, name = 'Vecinos', width = 4)%>%
  add_trace(x = ~año, y = ~AH, name = 'AA.HH.', width = 4)%>%
  add_trace(x = ~año, y = ~VV, name = 'Grupos de Vivienda', width = 4)%>%
  add_trace(x = ~año, y = ~MA, name = 'Ambientales', width = 4)%>%
  add_trace(x = ~año, y = ~SR, name = 'Adulto Mayor', width = 4)%>%
  add_trace(x = ~año, y = ~VL, name = 'Vaso de Leche', width = 4)%>%
  add_trace(x = ~año, y = ~OO, name = 'Otros', width = 4)%>%
  add_trace(x = ~año, y = ~AG, name = 'Agropecuarias', width = 4)%>%
  add_trace(x = ~año, y = ~OB, name = 'Obras y Vías', width = 4)%>%
  add_trace(x = ~año, y = ~TB, name = 'Trabajadores', width = 4)%>%
  add_trace(x = ~año, y = ~GE, name = 'Género', width = 4)%>%
  add_trace(x = ~año, y = ~MN, name = 'Municipalidad', width = 4)%>%
  add_trace(x = ~año, y = ~PR, name = 'Propietarios', width = 4)%>%
  add_trace(x = ~año, y = ~CD, name = 'Coordinadoras', width = 4)%>%
  add_trace(x = ~año, y = ~EM, name = 'Empresas', width = 4)%>%
  add_trace(x = ~año, y = ~SG, name = 'Seguridad', width = 4)%>%
  add_trace(x = ~año, y = ~FA, name = 'Familias', width = 4)%>%
  add_trace(x = ~año, y = ~ED, name = 'Educación', width = 4)%>%
  add_trace(x = ~año, y = ~HC, name = 'Discapacidad', width = 4)%>%
  add_trace(x = ~año, y = ~RE, name = 'Religión', width = 4)%>%
  add_trace(x = ~año, y = ~SA, name = 'Salud', width = 4)%>%
  add_trace(x = ~año, y = ~CT, name = 'Cultura', width = 4)%>%
  add_trace(x = ~año, y = ~media, name = 'Media', width = 4)%>%
  add_trace(x = ~año, y = ~mediana, name = 'Mediana', width = 4)
fig2 <- fig2 %>% layout(title = 'Variación de Participación por Tipo de Organización', plot_bgcolor = "#e5ecf6")
fig2 <- fig2 %>% layout(xaxis = list(title = 'Proceso de Presupuesto Participativo'))
fig2 <- fig2 %>% layout(yaxis = list(title = 'Agentes Participantes'))
fig2
```

```{r}
library(plotly)
fig3 <- plot_ly(TablaGraf1, type = 'scatter', mode = 'lines+markers')%>%
  add_trace(x = ~año, y = ~UR, name = 'Urbano', width = 4)%>%
  add_trace(x = ~año, y = ~CP, name = 'Comedores', width = 4)%>%
  add_trace(x = ~año, y = ~CM, name = 'Club de Madres', width = 4)%>%
  add_trace(x = ~año, y = ~AH, name = 'AA.HH.', width = 4)%>%
  add_trace(x = ~año, y = ~VL, name = 'Vaso de Leche', width = 4)
  #add_trace(x = ~año, y = ~media, name = 'Media', width = 4)%>%
...
fig3 <- fig3 %>% layout(title = 'Variación de Participación por Tipo de Organización', plot_bgcolor = "#e5ecf6")
fig3 <- fig3 %>% layout(xaxis = list(title = 'Proceso de Presupuesto Participativo'))
fig3 <- fig3 %>% layout(yaxis = list(title = 'Total de Agentes Participantes'))
fig3
```
