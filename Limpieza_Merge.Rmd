...
PP_FULL1 <- PP_FULL %>%
  mutate(
    orgn_mod = case_when(index_OG_col == 2040 | index_OG_col == 2041 | index_OG_col == 2503 | index_OG_col == 2625 ~ "AH001.0", #Asentamiento 1ro de Mayo
                         ...
                         index_OG_col == 2704 | index_OG_col == 2802 | index_OG_col == 2733 | index_OG_col == 2818 ~ "AH023.0", #Asentamiento Humano San Ignacio de Loyola - Lomo de Corvina
...
PP_FULL2 <- PP_FULL %>%
  mutate(tipo_orgn_mod =
           case_when(
             str_detect(orgn, "URBANIZACION") | str_detect(orgn, "URB.") | str_detect(orgn, "SECTOR&GRUPO") ~ 'UR',
             str_detect(orgn, "ASENTAMIENTO") ~ 'AH',
             str_detect(orgn, "AGR. ") | str_detect(orgn, "AGROPECUARIO") | str_detect(orgn, "AGROPECUARIA") ~ 'AG',
             str_detect(orgn, "VASO ") ~ 'VL', 
             str_detect(orgn, "PROPIETARIOS") | str_detect(orgn, "ADJUDICATARIOS")| str_detect(orgn, "POSECIONARIOS") ~ 'AP',
             str_detect(orgn, "FAMILIAS") | str_detect(orgn, "FAMILIAR") | str_detect(orgn, "VECINOS") | str_detect(orgn, "VECINA") ~ 'FV',
             str_detect(orgn, "AMBIENTE") | str_detect(orgn, "AREAS&VERDES") ~ 'MA',
             str_detect(orgn, "CLUB DE MADRES") ~ 'CM',
             str_detect(orgn, "COMEDOR") ~ 'CP',
             str_detect(orgn, "ADULTO MAYOR") ~ 'SR',
             str_detect(orgn, "DISCAPACIDAD") ~ 'HC'
             ),
...
```{r}
PP_SUBSET2 <- PP_FULL2[,c(10,4:8)]
distinct(PP_SUBSET2, tipo_orgn_mod, AÑO2015, AÑO2016, .keep_all = TRUE)
```

Juntar filas según etiqueta de organizaciones / tipo de organizaciones
```{r}
PP_SUBSET2[,c(2:6)] <- sapply(PP_SUBSET2[c(2:6)],as.numeric)
Resumen_Tipo_Org<- aggregate(. ~ tipo_orgn_mod, data = PP_SUBSET2, sum)
```
